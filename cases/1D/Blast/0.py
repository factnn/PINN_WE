...
def train(epoch, stage='Adam', total_epochs=None):
    ...
    def closure():
        ...
        # ========== 其他损失项（已注释，用于对比测试） ==========
        # loss_rh = actual_model.loss_rh(xrh,xrhL) # RH relation
        # loss_con = actual_model.loss_con(x_en,x_ic,crhoL,cuL,cpL,crhoR,cuR,cpR,Te-Ts) #Conservation laws
        # loss = loss_pde + 10*(loss_ic) + 10*(loss_rh) +10*loss_con  # PINN-WE配置
...
base_output_dir = create_output_dir('Sod', gpu_count=num_gpus, tag='baseline')
