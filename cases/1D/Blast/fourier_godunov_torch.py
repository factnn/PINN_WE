...
from fourier_features import FourierFeatureEncoder
import torch
import numpy as np
...
def train(epoch, stage='Adam', total_epochs=None, godunov_backend='torch', lambda_godunov=10.0):
    """
    训练函数（带Godunov损失）
    - Godunov损失：使用utility中的loss_godunov_euler函数
    - 只在激波区域（相邻点速度差大）应用Godunov loss
    """
    ...
    def closure():
        ...
        # Godunov损失（使用utility中的通用函数）
        # 注意：移除了 torch.cuda.synchronize() 以提升性能

        loss_godunov_val = loss_godunov_euler(
            actual_model, x_int,
            backend=godunov_backend,
            lambda_godunov=1.0,  # 这里设置为1.0，外部用lambda_godunov缩放
            gamma=1.4
        )
...
# 保存numpy版本的PDE残差点（用于Godunov损失）
x_int_np = x_int.copy()  # 保存numpy版本
x_int_sorted_np = np.sort(x_int_np, axis=0)  # 排序后的点（用于Godunov损失）
...
# ========== Fourier Features 编码器 ==========
fourier_encoder = FourierFeatureEncoder(input_dim=2, num_frequencies=32, sigma=1.0).to(cuda).double()
print(f'使用 Fourier Features: num_frequencies=32, sigma=1.0')

# ========== 网络结构（原始仓库参数） ==========
# 论文参数（已注释）:
# model = PINNs_WE_Euler_1D(Nl=8, Nn=50).to(cuda).double()  # 论文: 7层隐藏层，每层50个神经元
# 注意：使用Fourier features后，输入维度从2变为2*2*32=128
model = PINNs_WE_Euler_1D(Nl=6, Nn=60, fourier_encoder=fourier_encoder).to(cuda).double()
...
base_output_dir = create_output_dir('Sod', gpu_count=num_gpus, tag='fourier_godunov_torch')
...
# ========== Adam阶段：PINN + Godunov (Torch) ==========
print('========== Adam阶段：PINN + Godunov (Torch, k=0.2) ==========')
checkpoint_interval_stage1 = 1000  # Adam阶段每1000个epoch保存一次
print_interval = 100  # 每100个epoch打印一次，避免GPU同步影响性能
tic = time.time()
for epoch in range(1+epochi, epochs+epochi):
    loss = train(epoch, stage='Adam', total_epochs=epochs, godunov_backend='torch', lambda_godunov=10.0)

    # 每100个epoch打印一次，避免频繁GPU同步
    if epoch % print_interval == 0:
        print(f'Epoch {epoch}/{epochs}: loss_tot={loss:.8f}')
...
# ========== LBFGS阶段：PINN + Godunov (Torch) ==========
print('========== LBFGS阶段：PINN + Godunov (Torch, k=0.2) ==========')
epochs = 5000  # 原始仓库: 5000 epochs
checkpoint_interval_stage2 = 100  # LBFGS阶段每100个epoch保存一次
print_interval = 100  # 每100个epoch打印一次
tic = time.time()
for epoch in range(epochi, epochs+epochi):
    loss = train(epoch, stage='LBFGS', total_epochs=epochs, godunov_backend='torch', lambda_godunov=10.0)
...
result_file = 'result_fourier_godunov_torch.dat'
with open(result_file, 'w') as f:
    f.write('========== 实验配置 ==========\n')
    f.write(f'模式: Fourier_Godunov_Torch\n')
    f.write(f'Fourier Features: num_frequencies=32, sigma=1.0\n')
    f.write(f'Godunov后端: torch\n')
    f.write(f'Godunov权重: 10.0\n')
    f.write(f'权重函数k: 0.2\n')
    f.write(f'================================\n')
