...
print(f'GPU数量: {torch.cuda.device_count()} (当前代码使用单GPU)')
epoch = 0
epochi = epoch
...
t = np.full_like(x, Te)  # 在最终时刻 t=Te
...
print('结果已保存到: Sod_PINN_results.png')
plt.show()

# 保存数据
results = np.column_stack([x, rho_pred, p_pred, u_vel_pred])
np.savetxt('Sod_PINN_results.dat', results, 
           header='x rho p u', fmt='%.6e')
print('数据已保存到: Sod_PINN_results.dat')
