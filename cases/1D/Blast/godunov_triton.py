...
def train(epoch, stage='Adam', total_epochs=None, godunov_backend='triton', lambda_godunov=10.0):
    """
    训练函数（带Godunov损失）
    - Godunov损失：使用utility中的loss_godunov_euler函数（Triton后端）
    ...
    def closure():
        ...
        # Godunov损失（使用utility中的通用函数，Triton加速）
        # 注意：移除了 torch.cuda.synchronize() 以提升性能
        # 同步操作会破坏 GPU 异步执行，导致 Triton 变慢
...
base_output_dir = create_output_dir('Sod', gpu_count=num_gpus, tag='godunov_triton')
...
# ========== Adam阶段：PINN + Godunov (Triton) ==========
print('========== Adam阶段：PINN + Godunov (Triton, k=0.2) ==========')
...
for epoch in range(1+epochi, epochs+epochi):
    loss = train(epoch, stage='Adam', total_epochs=epochs, godunov_backend='triton', lambda_godunov=10.0)
...
# ========== LBFGS阶段：PINN + Godunov (Triton) ==========
print('========== LBFGS阶段：PINN + Godunov (Triton, k=0.2) ==========')
...
for epoch in range(epochi, epochs+epochi):
    loss = train(epoch, stage='LBFGS', total_epochs=epochs, godunov_backend='triton', lambda_godunov=10.0)
...
result_file = 'result_triton.dat'
with open(result_file, 'w') as f:
    f.write('========== 实验配置 ==========\n')
    f.write(f'模式: Godunov_Triton\n')
    f.write(f'Godunov后端: triton\n')
