...
def train(epoch):
    """
    训练函数（带Godunov损失）
    - Godunov损失：使用utility中的loss_godunov_euler函数（Torch后端）
...
output_dir = create_output_dir('Lax', base_dir=base_output_dir, gpu_count=gpu_count, tag='godunov_torch')
...
print('========== 第一阶段：Adam + Godunov (Torch, k=0.2) ==========')
...
if not interrupted:
    print('========== 第二阶段：LBFGS + Godunov (Torch, k=0.2) ==========')
