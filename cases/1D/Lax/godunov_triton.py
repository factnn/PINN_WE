...
# ========== 导入模块 ==========
from utility import save_results, select_gpu, create_output_dir, visualize_training_progress, visualize_initial_condition, loss_godunov_euler
...
def train(epoch):
    ...
    def closure():
        ...
        # Godunov损失（使用utility中的通用函数，Triton后端）
        import time as time_module
        start_godunov = time_module.time()
        ...
        godunov_time[0] += time_module.time() - start_godunov

        # 总损失
        loss = loss_pde + 10*loss_ic + 10.0*loss_godunov_val

        print(f'epoch {epoch} loss:{loss:.8f}, loss_pde:{loss_pde:.8f}, loss_ic:{loss_ic:.8f}, loss_god:{loss_godunov_val:.8f}')
        loss.backward()
        return loss

    loss = optimizer.step(closure)
    return loss, godunov_time[0]
...
class DNN(nn.Module):
    ...
    def loss_pde(self, x,xL,xR,Dx):
       ...
       # p_t, p_x = dp_g[:, :1], dp_g[:, 1:]                           
        ...
        # 
        # ========== Godunov实验配置：k=0.2 ==========
        # k=0.2时，在压缩区域适度降低权重，配合Godunov损失
        # 在压缩区域（u_x<0）时：d = (0.2*2*abs(u_x)+1) > 1，权重降低
...
output_dir = create_output_dir('Lax', base_dir=base_output_dir, gpu_count=gpu_count, tag='godunov_triton')
