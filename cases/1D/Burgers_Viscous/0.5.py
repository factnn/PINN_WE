...
def train(epoch, stage='Adam', total_epochs=None):
    """
    标准PINN训练函数（粘性Burgers方程）
    - 损失函数：loss_pde + loss_ic（无边界条件）
...
print(f'采样完成：初始条件{len(x_ic)}，内部点{len(x_int)}')
...
# ========== 创建输出目录 ==========
base_output_dir = create_output_dir('Burgers_Viscous', gpu_count=num_gpus, tag=f'baseline_nu{nu}')
print(f'\n[输出目录] {base_output_dir}')

# ========== Adam阶段训练 ==========
print(f'========== Adam阶段：标准PINN训练（k=0，粘性系数nu={nu}） ==========')
tic = time.time()
