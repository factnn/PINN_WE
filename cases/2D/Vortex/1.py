...
from utility import save_results, evaluate_predictions, select_gpu  # 统一工具
...
def train(epoch):
    ...
    def closure():
        optimizer.zero_grad()
        loss_pde = actual_model.loss_pde(x_int_train)
...
optimizer = torch.optim.LBFGS(model.parameters(), lr=lbfgs_lr, max_iter=lbfgs_max_iter)
loss_history = []
interrupted = False
stage_boundary = 0
tic_total = time.time()
...
output_dir, metrics = save_results(
    ...
    exact_dict=None,  # 评估/绘图改为自定义 2D 方式
    training_time=training_time,
    gpu_count=num_gpus,
    tag=tag,
    stage_boundary=stage_boundary,
    save_plot=False  # 关闭 1D 画法，改用与 notebook 一致的 2D 画法
)

# ====== 按 notebook 方式绘制 rho 对比（使用 vortex.dat 的 exact 列）======
vortex_dat = os.path.join(current_file_dir, 'vortex.dat')
if os.path.exists(vortex_dat):
    try:
        raw = np.loadtxt(vortex_dat, skiprows=3)  # TITLE/VARIABLES/ZONE 三行
        if raw.shape[1] >= 4:
            x_exact = raw[:, 0]
            y_exact = raw[:, 1]
            rho_num = raw[:, 2]      # 数值解（原文件中的 rho 列）
            rho_exact = raw[:, 3]    # 精确解（原文件 exact 列）
            # 依据文件头：I=100, J=100，与 notebook 相同的 100x100 网格
            rho_num_grid = rho_num.reshape(nx, ny)
            rho_exact_grid = rho_exact.reshape(nx, ny)
            rho_pred_grid = rho_pred  # 已是 (nx, ny)
            rho_err_grid = rho_pred_grid - rho_exact_grid

            fig, axes = plt.subplots(1, 3, figsize=(18, 5))
            im0 = axes[0].imshow(rho_pred_grid, origin='lower', extent=[Ly1, Ly, Lx1, Lx],
                                 cmap='viridis', aspect='equal')
            axes[0].set_title('PINN ρ (t=%.2f)' % Tend)
            fig.colorbar(im0, ax=axes[0], fraction=0.046, pad=0.04)

            im1 = axes[1].imshow(rho_exact_grid, origin='lower', extent=[Ly1, Ly, Lx1, Lx],
                                 cmap='viridis', aspect='equal')
            axes[1].set_title('Exact ρ (vortex.dat)')
            fig.colorbar(im1, ax=axes[1], fraction=0.046, pad=0.04)

            im2 = axes[2].imshow(rho_err_grid, origin='lower', extent=[Ly1, Ly, Lx1, Lx],
                                 cmap='RdBu_r', aspect='equal')
            axes[2].set_title('ρ Error (PINN - Exact)')
            ...
            plt.tight_layout()
            plot_path = os.path.join(output_dir, 'results_vortex.png')
            plt.savefig(plot_path, dpi=300, bbox_inches='tight')
            plt.close(fig)
            print(f'✓ 2D 结果图已保存（与 notebook 画法一致）: {plot_path}')
    except Exception as e:
        print(f'Warning: 读取/绘制 vortex.dat 失败: {e}')
