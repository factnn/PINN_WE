"""
1D Burgers CAN-PINN with Triton stencil kernel.
Drop-in replacement for baseline/burgers_1d.py — same network, same training,
only PDE residual computation swapped to Triton.
"""
import sys, time
...
import numpy as np
from kernels.stencil_1d import burgers_residual_triton
...
def train(model, epochs=5000, lr=1e-3, loss_threshold=1e-4):
    ...
    for ep in range(1, epochs + 1):
        t_step = time.time()
        opt.zero_grad()
        u = model(x.unsqueeze(1)).squeeze() if isinstance(model, MLP) else model(x)
        res = burgers_residual_triton(u, dx)
    ...
    start_evt.record(); burgers_residual_triton(u_test, dx); end_evt.record()
    torch.cuda.synchronize()
...
if __name__ == "__main__":
    print("=== CAN-PINN (MLP) + Triton ===")
    torch.cuda.reset_peak_memory_stats()
    train(MLP().to(device))

    print("\n=== Phy-CNN + Triton ===")
    torch.cuda.reset_peak_memory_stats()
    train(PhyCNN1D().to(device))
