...
def loss_fn(model, xyz, X, Y, Z, dx, dy, dz):
    ...
    u_g, v_g, w_g = model(xyz_g)
    u_g = u_g.reshape(Nx,Ny,Nz); v_g = v_g.reshape(Nx,Ny,Nz); w_g = w_g.reshape(Nx,Ny,Nz)
    ...
    res_u = u_g*u_x + v_g*u_y + w_g*u_z - nu*(u_xx+u_yy+u_zz)
    res_v = u_g*v_x + v_g*v_y + w_g*v_z - nu*(v_xx+v_yy+v_zz)
    res_w = u_g*w_x + v_g*w_y + w_g*w_z - nu*(w_xx+w_yy+w_zz)
    return (res_u**2+res_v**2+res_w**2).mean() + 10*bc_loss(u_g, v_g, w_g)
