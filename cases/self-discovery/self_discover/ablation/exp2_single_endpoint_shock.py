#!/usr/bin/env python3
"""
Ablation 2a: 单端硬约束 — 只约束激波端

C(V) = Cs + (V - Vs) * net(V)

保证 C(Vs) = Cs，但 sonic端不保证。

预期结果: 可能收敛到错误alpha，因为sonic端条件只靠ODE残差间接满足。
"""
import sys
...
def train_shock_only(gamma, n, geometry, alpha_init=0.75, epochs=10000,
                     lr=1e-3, n_colloc=400, device="cuda"):
    ref_alpha = get_ref_alpha(gamma, n)
    model = ShockOnlyPINN(gamma, n, alpha_init).double().to(device)
    ...
    def compute_loss():
        ...
        # Soft约束sonic端 (因为没有硬约束)
        C_at_sonic = model(V0.detach().reshape(1, 1))
        loss_sonic_bc = (C_at_sonic - C0) ** 2

        return (loss_ode_far + loss_ode_sonic), loss_numer, loss_sonic_bc.squeeze()
    ...
    for ep in range(1, warmup + 1):
        opt1.zero_grad()
        l_ode, l_reg, l_sonic = compute_loss()
        loss = l_ode + 10.0 * l_reg + 100.0 * l_sonic
        loss.backward()
        opt1.step()
...
if __name__ == "__main__":
    ...
    for gamma, geo in DEFAULT_CONFIGS:
        ...
        print(f"\n{'='*60}")
        print(f"  Exp2a Shock-Only Hard Constraint: gamma={gamma}, {geo}")
        print(f"{'='*60}")
        results[key] = train_shock_only(gamma, n, geo, alpha_init=ALPHA_INIT[(gamma, n)], epochs=10000, device=device)

    print_summary_table(results, "Ablation 2a: Shock-Only Hard Constraint")
    save_results_json(results, "Ablation 2a: Shock-Only Hard Constraint", str(out_dir / "exp2a_shock_only.json"))
    plot_ablation_result(results, "Ablation 2a: Shock-Only Hard Constraint",
                         str(out_dir / "exp2a_shock_only.png"))
