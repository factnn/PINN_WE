...
预期结果: 可能不收敛，因为:
  1. 激波端条件只靠soft loss
  2. alpha的梯度只通过C0(alpha)传播，少了一半信息
"""
import sys
...
def train_sonic_only(gamma, n, geometry, alpha_init=0.75, epochs=10000,
                     lr=1e-3, n_colloc=400, device="cuda"):
    ref_alpha = get_ref_alpha(gamma, n)
    model = SonicOnlyPINN(gamma, n, alpha_init).double().to(device)
    ...
    def compute_loss():
        ...
        loss_shock_bc = (C_at_shock - Cs) ** 2

        return (loss_ode_far + loss_ode_sonic), loss_numer, loss_shock_bc.squeeze()
    ...
    for ep in range(1, warmup + 1):
        opt1.zero_grad()
        l_ode, l_reg, l_shock = compute_loss()
        loss = l_ode + 10.0 * l_reg + 100.0 * l_shock
        loss.backward()
        opt1.step()
...
if __name__ == "__main__":
    ...
    for gamma, geo in DEFAULT_CONFIGS:
        ...
        print(f"\n{'='*60}")
        print(f"  Exp2b Sonic-Only Hard Constraint: gamma={gamma}, {geo}")
        print(f"{'='*60}")
        results[key] = train_sonic_only(gamma, n, geo, alpha_init=ALPHA_INIT[(gamma, n)], epochs=10000, device=device)

    print_summary_table(results, "Ablation 2b: Sonic-Only Hard Constraint")
    save_results_json(results, "Ablation 2b: Sonic-Only Hard Constraint", str(out_dir / "exp2b_sonic_only.json"))
    plot_ablation_result(results, "Ablation 2b: Sonic-Only Hard Constraint",
                         str(out_dir / "exp2b_sonic_only.png"))
