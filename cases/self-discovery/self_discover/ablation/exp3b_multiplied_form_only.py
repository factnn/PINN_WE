#!/usr/bin/env python3
"""
Ablation 3b: 纯乘法形式残差 — dC/dV * denom - numer

成功方案用混合形式: 远离奇点用除法，靠近奇点用乘法。
本实验: 全部用乘法形式。

预期结果: 可以训练（不会NaN），但alpha精度差，因为：
  乘法形式在远离奇点处，denom很大 → 残差被放大 → 梯度信号不均匀
  等效于给不同区域加了不均匀权重
"""
import sys
...
def train_multiplied_only(gamma, n, geometry, alpha_init=0.75, epochs=10000,
    ...
    def compute_loss():
        ...
        # 加numer正则 (保持和成功方案一致)
        loss_numer = torch.mean(numer ** 2)

        return loss_ode, loss_numer
...
if __name__ == "__main__":
    ...
    for gamma, geo in DEFAULT_CONFIGS:
        ...
        print(f"\n{'='*60}")
        print(f"  Exp3b Multiplied-Form Only: gamma={gamma}, {geo}")
        print(f"{'='*60}")
        results[key] = train_multiplied_only(gamma, n, geo, alpha_init=ALPHA_INIT[(gamma, n)], epochs=10000, device=device)

    print_summary_table(results, "Ablation 3b: Multiplied-Form Only")
    save_results_json(results, "Ablation 3b: Multiplied-Form Only", str(out_dir / "exp3b_multiplied_only.json"))
    plot_ablation_result(results, "Ablation 3b: Multiplied-Form Only",
                         str(out_dir / "exp3b_multiplied_only.png"))
