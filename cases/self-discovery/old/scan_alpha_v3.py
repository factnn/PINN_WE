...
Guderley 漏斗扫描实验 V3 (使用修正后的硬约束边界条件)

修正内容:
1. 使用精确RH条件 (M∞=10): V_bc=0.175, C_bc=0.697
2. model.py 已修正硬约束边界条件
3. 二阶ODE完整实现

目标：验证损失函数在α=0.717处是否最小
"""
...
class AlphaScanExperimentV3:
    ...
    def run_scan(self):
        ...
        print("=" * 60)
        print("Guderley 漏斗扫描实验 V3（修正硬约束边界条件）")
        print("=" * 60)
        print(f"α候选值: {self.alpha_values}")
        print(f"真实α值: 0.717")
        print(f"训练策略: Adam(5000) + L-BFGS(2000)")
        print(f"马赫数: M∞={self.mach_inf}")
        print(f"边界条件: V_bc=0.175, C_bc=0.697 (精确RH)")
    ...
    def plot_results(self):
        ...
        ax1.axvline(x=0.717, color='r', linestyle='--', label='True alpha=0.717')
        ax1.set_xlabel('alpha', fontsize=12)
        ax1.set_ylabel('Final Loss', fontsize=12)
        ax1.set_title('Loss vs alpha (Linear) - V3 Corrected Hard BC', fontsize=14)
        ax1.legend()
        ax1.grid(True, alpha=0.3)

        # 图2: 对数尺度
        ax2.semilogy(alphas, losses, 'b-o', linewidth=2, markersize=8)
        ax2.axvline(x=0.717, color='r', linestyle='--', label='True alpha=0.717')
        ax2.set_xlabel('alpha', fontsize=12)
        ax2.set_ylabel('Final Loss (log scale)', fontsize=12)
        ax2.set_title('Loss vs alpha (Log) - V3 Corrected Hard BC', fontsize=14)
        ax2.legend()
        ax2.grid(True, alpha=0.3)

        plt.tight_layout()

        plot_file = self.output_dir / "funnel_plot_v3.png"
    ...
    def analyze_results(self):
        ...
        if loss_at_true is not None:
            ...
            if abs(min_alpha - 0.717) < 0.05:
                ...
                print("    1. 损失函数权重需要调整")
                print("    2. 网络容量不足")
                print("    3. 需要更多物理约束（熵条件等）")
                print("    4. PINN方法本身的局限性")
...
def main():
    ...
    # 创建实验
    experiment = AlphaScanExperimentV3(
        alpha_values=alpha_values,
        gamma=1.4,
        n=2,
        n_interior=200,
        mach_inf=10.0,
        output_dir="output_scan_v3"
    )
