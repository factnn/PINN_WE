...
Guderley 漏斗扫描实验 V4 (测试不同损失函数权重)

修正内容:
1. 使用修正后的硬约束 (V_bc=0.175, C_bc=0.697)
2. 测试不同的权重组合
...
class WeightScanExperiment:
    """权重扫描实验"""

    def __init__(
        self,
        alpha_values: list,
        weight_configs: list,  # [(weight_pde, weight_bc_center), ...]
        gamma: float = 1.4,
        n: int = 2,
        n_interior: int = 200,
        mach_inf: float = 10.0,
        output_dir: str = "output_scan_v4"
    ):
        ...
        # 存储结果
        self.all_results = {}  # {weight_config: results}
    ...
    def run_weight_scan(self):
        ...
        print("=" * 60)
        print("Guderley 权重扫描实验 V4")
        print("=" * 60)
        print(f"α候选值: {self.alpha_values}")
        print(f"权重配置数: {len(self.weight_configs)}")
        print(f"训练策略: Adam(3000) + L-BFGS(1000) [快速版]")

        for config_idx, (w_pde, w_center) in enumerate(self.weight_configs, 1):
            print(f"\n\n{'='*60}")
            print(f"[配置 {config_idx}/{len(self.weight_configs)}]")
            print(f"weight_pde={w_pde}, weight_bc_center={w_center}")
            print(f"{'='*60}")
            ...
            for i, alpha in enumerate(self.alpha_values, 1):
                ...
                final_loss = self.train_fixed_alpha(
                    alpha, w_pde, w_center,
                    adam_epochs=3000, lbfgs_epochs=1000, print_every=1000
                )
            ...
            # 保存这个配置的结果
            config_key = f"pde{w_pde}_center{w_center}"
            self.all_results[config_key] = results

            # 分析这个配置
            self.analyze_config(config_key, results, w_pde, w_center)

        # 保存所有结果
        self.save_all_results()
        self.plot_all_configs()

    def analyze_config(self, config_key, results, w_pde, w_center):
        """分析单个权重配置的结果"""
    ...
    def save_all_results(self):
        """保存所有配置的结果"""
        results_file = self.output_dir / "weight_scan_results.json"
    ...
    def plot_all_configs(self):
        ...
        for idx, (config_key, results) in enumerate(self.all_results.items()):
            ...
            axes[idx].set_title(f'{config_key}', fontsize=12)
            axes[idx].legend()
            axes[idx].grid(True, alpha=0.3)

        plt.tight_layout()
        plot_file = self.output_dir / "weight_comparison.png"
...
def main():
    ...
    # 定义权重配置 (weight_pde, weight_bc_center)
    weight_configs = [
        (10.0, 1.0),   # 配置1: 增大PDE权重，减小Center权重
        (20.0, 0.5),   # 配置2: 进一步增大PDE权重
        (50.0, 0.1),   # 配置3: 极大PDE权重，极小Center权重
    ]
    ...
    print(f"\n权重配置:")
    for i, (w_pde, w_center) in enumerate(weight_configs, 1):
        print(f"  配置{i}: weight_pde={w_pde}, weight_bc_center={w_center}")
    print(f"\n对比基线 (V3): weight_pde=1.0, weight_bc_center=5.0")

    # 创建实验
    experiment = WeightScanExperiment(
        ...
        n_interior=200,
        mach_inf=10.0,
        output_dir="output_scan_v4"
    )

    # 运行扫描
    experiment.run_weight_scan()
